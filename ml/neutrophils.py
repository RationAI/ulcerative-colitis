...
def download_data(datasets: ListConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for dataset in datasets:
        ...
        tissue_masks = Path(mlflow.artifacts.download_artifacts(uris.tissue_masks))
        ...
        for split, df in zip(
            ["train", "test_preliminary"], [train_dfs, test_dfs], strict=False
        ):
            if split in uris.tiling:
                df.append(
                    pd.read_parquet(
                        Path(mlflow.artifacts.download_artifacts(uris.tiling[split]))
                        / "slides.parquet",
                    )
                    .set_index("id")
                    .assign(
