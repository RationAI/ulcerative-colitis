...
def get_slides(artifact_uri: str) -> list[Path]:
    path = mlflow.artifacts.download_artifacts(artifact_uri)
    return [Path(p) for p in pd.read_parquet(path)["path"]]


def main() -> None:
    train_slides = get_slides(
        "mlflow-artifacts:/27/b343ca345c1442c1a92ae20de697a91c/artifacts/Ulcerative Colitis - train/slides.parquet"
    )
    val_slides = get_slides(
        "mlflow-artifacts:/27/b343ca345c1442c1a92ae20de697a91c/artifacts/Ulcerative Colitis - val/slides.parquet"
    )
    test1_slides = get_slides(
        "mlflow-artifacts:/27/b343ca345c1442c1a92ae20de697a91c/artifacts/Ulcerative Colitis - test1/slides.parquet"
    )
    test2_slides = get_slides(
        "mlflow-artifacts:/27/b343ca345c1442c1a92ae20de697a91c/artifacts/Ulcerative Colitis - test2/slides.parquet"
    )
